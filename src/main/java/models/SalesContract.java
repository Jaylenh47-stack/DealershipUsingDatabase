    ...
    public double getMonthlyPayment(){
        ...
        if (isFinanced){
            if (this.getVehicleSold().getPrice() > 10000){
                return Calculations.calculateMonthlyPayment(this.getVehicleSold().getPrice(), 0.0425f, 48);
            }
            else{
                return Calculations.calculateMonthlyPayment(this.getVehicleSold().getPrice(), 0.0525f, 24);
            }
        }
        else{
            return 0;
